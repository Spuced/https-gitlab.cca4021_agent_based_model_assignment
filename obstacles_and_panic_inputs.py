...
# Creating and placing input fields with labels
def create_labeled_input(root, label, default_value, pack=True):
    frame = tk.Frame(root)
...
default_params = {
    ...
    "Number of Fires": 1,
    "Fire Spread Rate": 0.4,
    "Vision": 10
}

# Use the function to create input fields
panic_percent_input = create_labeled_input(root, "Panic Percent", default_params["Panic Percent"])
number_of_workers_input = create_labeled_input(root, "Number of Workers", default_params["Number of Workers"])
number_of_fires_input = create_labeled_input(root, "Number of Fires", default_params["Number of Fires"])
fire_spread_rate_input = create_labeled_input(root, "Fire Spread Rate", default_params["Fire Spread Rate"])
vision_input = create_labeled_input(root, "Vision", default_params["Vision"])
...
def get_input_value(entry, cast_type):
    try:
...
def update_parameters():
    ...
    panic_percent = get_input_value(panic_percent_input, float)
    number_of_workers = get_input_value(number_of_workers_input, int)
    number_of_fires = get_input_value(number_of_fires_input, int)
    fire_spread_rate = get_input_value(fire_spread_rate_input, float)
    vision = get_input_value(vision_input, int)

def run_continuous():
    update_parameters()
    if all(value is not None for value in [panic_percent, number_of_workers, number_of_fires, fire_spread_rate, vision]):
        update()
        canvas.delete("all")
        draw_grid(canvas)
        worker_label.config(text="Workers Escaped: " + str(escaped_workers))
        dead_label.config(text="Workers Died: " + str(dead_workers))
        root.after(50, run_continuous)  # Repeat the function to continuously run
    else:
